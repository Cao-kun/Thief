...
def ExecuteCommandsFile(path):
    ...
    if path[2] != ":":
        ...
        path = os.path.join(str(path, encoding="utf-8"))
    # 打开文件
    with open(path) as f:
        # 将记录的命令写入命令列表
    ...
    for command in command_list:
        ...
        #print(command[1][2])
        print(command[2])
        if command[0].decode('utf8') == "click":
            # 将鼠标移动到记录中的位置
            ...
            time.sleep(0.1)
            # 点击
            mouse.click(buttons[command[1][2].decode('utf8')])
        # 如果是按键按下
        elif command[0].decode('utf8')  == "press":
        ...
        # 如果是按键释放
        elif command[0].decode('utf8')  == "release":
